...
var data,tr;
function init(page){
    ajax_method(map.localurl+map.queryclubcardrecord,'token='+token+'&type=转赠&page='+page+'&size=15','post',function successCallBack(a){
...
window.onscroll = function() {
    if(getScrollTop() + getClientHeight() == getScrollHeight()) {
        vid++;
        ajax_method(map.localurl+map.queryclubcardrecord,'token='+token+'&type=转赠&page='+vid+'&size=15','post',function successCallBack(a){
            data = JSON.parse(a).data.items;
