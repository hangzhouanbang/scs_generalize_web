...
function init(){
    ajax_method(map.localurl+map.info,
        ...
        var html3 = '<tr>\n' +
            ...
            '    </tr>'+
            '    <tr>\n' +
            '        <td><img src="../img/icon_jade.png" style="width: 0.66rem;height:0.9rem;"></td>\n' +
            '        <td>'+data.coins+'</td>\n' +
            '        <td>\n' +
            '            <input type="button" value="充值账号" class="btn1 recharge" id="yu"\n' +
            '                   onclick="recharge(this)"/><br/>\n' +
            '        </td>\n' +
            '    </tr>'
...
function sure(){
    ...
    ajax_method(map.localurl + map.confirm, 'memberId='+obj.memberId, 'post', function successCallBack(h){
        console.log(JSON.parse(h))
        if(JSON.parse(h).success){
            if(obj.card == '玉石'){
                var html4 = ' <div class="headline">\n' +
                    '                <p>请确认充值内容</p>\n' +
                    '                <p>玩家：'+obj.memberId+'</p>\n' +
                    '                <p>昵称：'+JSON.parse(h).data.name+'</p>\n' +
                    '                <p>'+obj.card+'：'+obj.number+'个</p>\n' +
                    '            </div>\n' +
                    '            <div class="querycard">\n' +
            ...
            show(document.getElementsByClassName('mask8'),document.getElementsByClassName('mask1'))
        }
